...
function contentMine(chosenWords){
    ...
    for(var i=0; i < sortablePhrases.length; i++)
    ...
    var sortedBigrams = generateBigrams(chosenWordsSplit, sortable, 2, SAMPLING_RATE)
	
    var ngrams = JSON.parse(localStorage["ngrams"])
    
    for(var i=0; i < sortedBigrams.length * BEGINNER; i++)
...
// Generates ngrams
function generateBigrams(words, wordFreq, n, SAMPLING_RATE){
    ...
    // Sampling rate limits the number of bigrams generated. Monte Carlo sample
    for(var i = 0; i < words.length * SAMPLING_RATE; i++){
